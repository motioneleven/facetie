module Facets
  class Facets::MultiplePresenter < FacetPresenter
    ...
    def facet_link_tag
      link_to(text, view.url_for(params.merge(filter => ((params[filter] || []) + [value]))), class: 'facet-link')
    end

    def facet_remove_tag
      content_tag(:span, class: 'facet-remove') do
        link_to(content_tag(:i, nil, class: 'icon-remove'), view.url_for(params.merge(filter => (params[filter] - [value]))), class: 'facet-link')
      end
    end
